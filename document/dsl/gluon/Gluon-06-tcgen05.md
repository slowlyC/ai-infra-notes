...
Tensor memory 是一个二维内存空间, 在每个 CTA 上有 128 行 × 512 列的单元，每个单元为 32 bit。
...
- 数据可以从共享内存异步复制到 Tensor memory, 但这个 API 目前尚未在 Gluon 中开放。
...
对于小于 32 位的数据类型, 如果设置 unpacked=False, 可以将多个元素打包到同一个 32 位单元中, 
但此时 blockN 必须至少为 `32 // bitwidth`。 例如, 对于 fp16 类型, 至少为`32 // 16 = 2`。

注意:当 blockM=64 时, 包含多个块的 tensor 会在 TMEM 中被打包以充分利用物理上的全部 128 行。
这可能会使 TMEM 描述符的切片操作变得复杂。

底层的 `tcgen05.st` 和 `tcgen05.ld` 指令是 warp 级别的指令, 以特定模式访问 TMEM。
结合 warp 的行寻址限制, 这对 Tensor memory 的寄存器 layout 产生了约束。
某些 Tensor memory layout 支持多种寄存器 layout, 这会影响原子操作的选择。
在本教程中, 我们只使用 `32x32b` 原子: 即每个 lane 存储和load 1 行 TMEM。

### tmem用法

下面我们来演示如何使用 Tensor memory 对二维数据进行读写。

input: global memory -> 寄存器 -> Tensor memory -> output:寄存器 -> global memory
...
def small_mma_kernel(a_desc, b_desc, c_desc, d_desc, tmem_block: gl.constexpr,  #
    ...
    # # 先将c_smem从SMEMload到寄存器, 再从寄存器复制到 acc TMEM 中。
    # # TODO: 等 Gluon 中开放 `tcgen05.cp` 指令后使用它。tcgen05_copy？
    ...
    if LHS_IN_TMEM:
        ...
        # 注意: tcgen05_copy 只支持 32-bit 元素类型，而 A 矩阵是 float16，
        # 所以 LHS 只能使用寄存器中转方式。
    ...
    # ```
    #
    # 在共享内存写入和 tcgen05_mma 之间需要 fence 来保证它们的内存访问顺序。
    # tcgen05_mma 操作完成意味着它对共享内存的读取已经结束, 此时才可以安全地写入共享内存。
    #
    # tcgen05_mma 操作的完成由 mbarrier 跟踪。在 mbarrier 上调用 tcgen05_commit
    # 会使得所有先前发出的 tcgen05_mma 操作完成时, mbarrier 被 arrive(到达)。
    # 关于 mbarrier 工作原理的更多细节, 请参阅 04-tma.py。
    #
    # 要提交到 mbarrier, 我们可以显式调用 tcgen05_commit, 或者将mbarrier直接传递给tcgen05_mma。
    # 如果需要, 还可以有条件地提交 mbarrier。
    #
    # tcgen05_mma 由多个异步 MMA 指令组成。每个指令的形状由 TMEM layout 决定。
    # 选择更大的指令形状通常能获得更好的性能。
...
让我们对分块矩阵乘法 kernel 进行基准测试。关于超参数选择的更多信息, 

请参考前一个教程 05-wgmma.py。
...
由于 tcgen05_mma 是异步的, 我们可以将它与 TMA load重叠, 从而减少 SM 的空闲时间。

虽然 tcgen05 指令是异步的, 但它们具有"隐式流水线"特性, 即在以下情况下,

硬件会自动保证指令按发出顺序执行, 无需手动插入同步:

1. 两个以上具有相同形状和累加器dtype的 tcgen05_mma 指令
