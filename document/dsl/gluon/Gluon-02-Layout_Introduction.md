...
- **Gluon-02-Layout_Introduction**（本文）
...
在 Gluon 中，每个 tensor 都需要指定 layout。

Layout 描述了 tensor 的元素如何分布到线程块中的各个线程上。这种分布遵循 GPU 的层级结构：线程块 → warp（线程束）→ lane（通道/线程）→ 寄存器。Tensor 的元素在线程间均匀分布，即每个线程处理相同数量的元素。由于 Triton 要求所有分块维度都是 2 的幂，因此每个线程持有的元素数量也是 2 的幂。

Layout 本质上定义了一个映射关系：给定（寄存器编号、lane 编号、warp 编号），可以确定对应的 tensor 元素。

### BlockedLayout介绍

`BlockedLayout` 是 Gluon 中最常用的 layout 类型，它将元素组织成与 tensor 维度相同的"layout 块"。来看一个具体例子：
...
对于fp32数据类型, 一个寄存器能存储1个元素。而fp16数据类型, 一个寄存器能存储2个元素。

我们将 `size_per_thread`、`threads_per_warp` 和 `warps_per_cta` 逐元素相乘, 得到layout块形状为 [64, 16]。

`order` 指定了tensor维度的分块顺序。`order=[1, 0]` 表示先沿第 1 维（列）分块，再沿第 0 维（行）分块，即行优先顺序。

假设数据类型为fp32, 对于线程 T，其持有的元素分布如下（T:n 表示线程 T 的第 n 个寄存器）：
...
注意：warp 内的线程数必须是 32（NVIDIA 硬件规定）。
把每个线程替换成它持有的 2×4 子块，就得到整个 warp 持有的 tensor 区域：
...
`平铺(tile)`是指将layout块复制并重复, 以适应tensor的形状, 示例:
考虑一个 `128x64xf32` tensor。将tensor形状除以layout块形状[64, 16], 我们得到layout块的数量为 `[2, 4]`。
...
理解寄存器用量对于控制 kernel 的寄存器压力至关重要，计算下`128x64xf32` tensor的寄存器用量:
由于 `size_per_thread=[2, 4]`，每个线程在一个layout块内持有8个寄存器，
那么覆盖整个tensor需要8个layout块，因此每个线程共持有8x8=64个寄存器。

`广播(broadcast)`是指将tensor的元素复制并重复, 以适应layout块的形状, 示例:
...
```
位置 [0,0]：被 T0:0, T32:0, T64:0, T96:0 这4个线程持有
位置 [0,1]：被 T1:3, T33:3, T65:3, T97:3 这4个线程持有


BlockedLayout 是最常用的 layout，主要用于：
- 表示全局内存访问时的合并 layout
...
# %%
# 使用 layout 将工作分配到所有线程。
# 把 layout 作为参数传入 kernel，方便测试不同 layout 的效果。每个程序负责复制一个数据块。
...
当 R=1 时, 每个线程读取4字节(float32)。然而最小访问粒度是32字节，所以每个`LDG.E` 指令要读取32字节, 这大于一个线程需要的4字节.而一个线程束(warp)中的 32 个线程访问的是连续的 32*4=128 字节区域，因此硬件会将一个线程束(warp)中的 32 个线程的 `LDG.E` 访问合并成 128 字节进行访问, 这正好是一个缓存行的大小。
...
在 2D memcpy 中, 我们需要计算每个元素在内存中的位置。对于2D tensor, 我们需要:
...
现在, 如果我们想沿 dim=1(列维度)创建一个SliceLayout, 过程如下:
...
第二步: 删除每个线程内的重复寄存器映射, 只保留每个线程的第一个寄存器:
```
...
def memcpy_2d_impl(input, output, XBLOCK, YBLOCK, layout, num_warps):
    ...
    # 将输入和输出tensor的步幅传递给kernel。当步幅为 1 时(这在tensor的内部维度中很常见), 
...
def test_memcpy_2d(XBLOCK, YBLOCK, xnumel, ynumel, transposed, num_warps):
    ...
    output = torch.empty_like(input)
    # 转置tensor使其沿内部维度不连续，即每一行的元素在内存中不是连续的。
...
def bench_memcpy_2d(impl, transposed=False):
    # 8GB大小的tensor: [32*1024, 64*1024]
    xnumel = 32 * 1024
    ynumel = 64 * 1024
    input = torch.randn((xnumel, ynumel), device="cuda")
    output = torch.empty_like(input)
    # 转置tensor使其沿内部维度不连续, 即input.is_contiguous(): False
    input = input.T if transposed else input
    output = output.T if transposed else output
    return bench_memcpy_impl(input, output, impl)
...
性能的变化是由于不同的调度顺序影响了数据局部性, 

即使每个程序（thread block）访问的数据量和访问模式相同，但由于执行顺序不同，缓存命中率会有差异，从而影响性能。

在后续教程中, 我们将探索实现持久性kernel(persistent kernels)以及如何使用它们来更好地控制调度, 以提高性能。

### 对非连续tensor转置

从上面的数据可以看出, 在输入和输出tensor都连续时, 1D memcpy比2D memcpy 性能略好。然而, 当输入或输出的layout不连续或更奇形怪状时, 使用2D memcpy 会有更佳的表现。

举个例子: 假设我们有一个大小为8GB的tensor, 通过`x[::2]`获取每隔一行的视图, 作为输入tensor, 其是非连续的。
...
下面我们将这个非连续的输入tensor复制到一个连续的输出tensor中, 相当于 PyTorch 的 `x.contiguous()` 操作。
...
即使输入是非连续的，我们的 2D memcpy 仍能接近tensor连续时的性能，且比 PyTorch 的`contiguous()`快2倍多。
...
if __name__ == "__main__" and _enabled("memcpy_2d_inout"):
    ...
    # output沿 dim 0 连续。进行了转置操作。
    output = torch.empty((input.shape[1], input.shape[0]), device="cuda").T
    
    # 问题: 输入和输出具有相反的连续方向!
    # 无论我们选择哪种layout, 总有一个方向无法获得合并访问

    # 尝试1: 使用 order=[1, 0], 适合按行访问
    # 这对输入tensor很好(按行访问, 获得合并), 但对输出tensor很差(按行访问, 无法合并)
    layout = gl.BlockedLayout([1, 1], [1, 32], [1, 4], [1, 0])
    impl = partial(memcpy_2d_impl, XBLOCK=1, YBLOCK=2048, layout=layout, num_warps=4)
    _, throughput = bench_memcpy_impl(input, output, impl)
    ...
    # 这对输出tensor很好(按列访问, 获得合并), 但对输入tensor很差(按列访问, 无法合并)
    layout = gl.BlockedLayout([1, 1], [32, 1], [4, 1], [0, 1])
    impl = partial(memcpy_2d_impl, XBLOCK=2048, YBLOCK=1, layout=layout, num_warps=4)
    _, throughput = bench_memcpy_impl(input, output, impl)
...
1. 输入tensor选择适合行访问的layout(优化读取)
2. 输出tensor选择适合列访问的layout(优化写入)
3. 在写入输出tensor前进行layout转换

Layout转换的原因和成本:

从全局内存加载数据时需要输入tensor的layout, 但存储时需要输出tensor的layout。

因此, 当我们为输入和输出使用不同的layout时, 需要在两者之间执行layout转换。
...
不过，这个成本远低于低效全局内存访问的成本(无法合并访问)。因此, 使用layout转换是值得的。
...
**归约(Reductions)、扫描(Scans)、收集(Gathers)等操作**
...
注意: 编译器在生成高效的归约代码方面做得很好, 通常不需要为了归约而转换layout。但在可以选择的情况下, 选择高效的归约layout仍然是有益的。

**共享内存访问**

共享内存被组织成多个bank, 每个周期每个线程束只能访问一个bank地址。编译器会生成最小化bank冲突的代码, 但bank冲突的数量仍然受layout影响。
