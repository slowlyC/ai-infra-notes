...
在 CuTe 中还有几个重要的术语需要先了解：

- **rank**：Layout 的 mode 数量，即 Shape 最外层 tuple 的元素个数。例如 `(3, 4)` 的 rank 为 2。
- **depth**：Layout 的嵌套深度。标量的 depth 为 0，`(3, 4)` 的 depth 为 1，`(3, (2, 4))` 的 depth 为 2。
- **size**：逻辑空间的大小（domain），等于 Shape 所有叶子元素的乘积。
- **cosize**：物理空间的覆盖范围大小（codomain），定义为 `Layout(size - 1) + 1`，即最大 offset 加 1。

在介绍有层次的 Layout 之前，我们先回顾无层次的 Layout 描述，也就是 shape 和 stride 描述的高维 Tensor。

## 一维向量的表示

**Shape: (8), Stride: (1)** 表示该排列包含 8 个逻辑位置，在逻辑位置和物理（数据）做映射的时候每一个元素之间的差为 1，如图 1 所示其描述了 0-7 总计 8 个数字，其计算逻辑为 `offset = coord * stride`。

![Figure 1. shape = 8, stride = 1的逻辑空间和物理空间映射关系](images/cute1_layout_001.jpg)
*Figure 1. shape = 8, stride = 1 的逻辑空间和物理空间映射关系*

**Shape: (8), Stride: (2)** 表示该排列包含 8 个逻辑位置，每一个位置的坐标为自然序列 0-7，如图 2 所示，其对应物理位置映射时的公式为 `offset = coord * stride`。这时候我们发现逻辑空间和物理空间的大小是不一样的。在 CuTe 体系下，逻辑空间的大小称作 **size**（domain），而代表存储覆盖范围的物理空间大小称作 **cosize**（codomain）。对于这个例子：`size = 8`，最大 offset = (8-1) * 2 = 14，所以 `cosize = 14 + 1 = 15`。
...
以上一维空间的示例，呈现了 Tensor 在其 shape 不变的情况下，通过 stride 的改变可以描述 Tensor 中的各个元素在物理空间中的位置。我们在使用 Tensor 时候，关注的是其逻辑的大小，而通过 stride 则将这个逻辑空间和实际存储的物理空间进行了关联。并且在计算层面，我们可以看到其始终满足：
...
**Shape: (3, 4), Stride: (1, 3)**，和一维向量比较类似，此次的二维空间指的是 Tensor 的逻辑空间，其存储结构依然是一维的。二维空间的列优先描述可以表达为 `shape(3, 4), stride(1, 3)`，如图 5 所示。shape 中的 3、4 分别表示矩阵的行数和列数，stride 中的 1 表示沿行方向增加 1 步时物理地址增加 1，3 表示沿列方向增加 1 步时物理地址增加 3。

> 这里的 stride(1, 3) 说的是列优先（column-major），因为行方向（第一个维度）的 stride 为 1，也就是同一列内的相邻元素在内存中是连续的。

![Figure 5. shape = (3,4) stride = (1, 3)的逻辑空间和物理空间的映射关系](images/cute1_layout_005.jpg)
*Figure 5. shape = (3,4) stride = (1, 3) 的逻辑空间和物理空间的映射关系*

**Shape: (3, 4), Stride: (4, 1)**，Shape 不变，而 stride 由 (1, 3) 变为 (4, 1) 则存储结构变为行优先（row-major），即同一行内的相邻元素在物理存储中是连续的。

![Figure 6. shape = (3, 4) stride = (4, 1)的逻辑空间和物理空间的映射关系](images/cute1_layout_006.jpg)
*Figure 6. shape = (3, 4) stride = (4, 1) 的逻辑空间和物理空间的映射关系*

二维矩阵的描述和一维类似，shape 表示其逻辑形状，stride 表示具体的某个元素和物理空间的映射时的间隔量。逻辑空间到物理空间的映射通过点积来完成。从二维空间我们可以很容易地扩展到高维空间，如深度学习常用的 (N, H, W, C)，其映射关系依然利用点积公式：

$$
offset = coordinate \cdot stride = \sum_{i}{coordinate_{i} \cdot stride_{i} }
$$

## 有层次的 Layout（Hierarchical Layout）

以上介绍的一维向量和二维矩阵描述被深度学习框架所广泛采用，在 PyTorch 中我们可以访问 tensor 的 `.shape` 属性和 `.stride()` 方法来获取对应的信息。我们不难发现，上面的 shape 和 stride 描述其限制了 Tensor 的每一个轴只能有一个 stride 值，也就是说，整个 Tensor 在某一个维度上的连续性关系是不能变的，更形象地描述则为：Tensor 不可以分块。我们将这样轴的连续性不可变更、体现为 Tensor 不可以分块的描述称为平坦 Tensor 描述。而当我们处理复杂的 Tensor 计算问题时，尤其是如 NVIDIA 硬件引入的指令级计算（MMA、TMA 等）时，这种表示是不充分的。由此则引入了有层次的 Tensor 描述，即 Hierarchical Layout。
...
**第二步：识别外层排列。** 把每个红框看作一个元素，观察红框之间的排列。这些红框只在列方向上排列了 4 个（行方向只有 1 组）。第一个红框的起始 offset 是 0，第二个是 8，第三个是 16，第四个是 24，相邻红框之间的 offset 差为 8。

**第三步：合并层次。** 行方向上 shape 为 4（来自内层 tile 的行数），列方向上 shape 为 (2, 4)，其中 2 是内层 tile 的列数，4 是外层有 4 个 tile。对应的 stride：行方向为 2（内层行 stride），列方向为 (1, 8)（内层列 stride=1，外层 tile 间 stride=8）。

最终得到层次化的 Layout：**shape: (4, (2, 4)), stride: (2, (1, 8))**。

验证：访问坐标 (0, (1, 1))（即全局第 0 行第 3 列），offset = 0\*2 + 1\*1 + 1\*8 = 9，对应图中位置 9，正确。
...
**内层 tile（红框）：** 每个红框是 2 行 2 列的小矩阵，列优先排列。行方向 stride 为 1（如 0→1），列方向 stride 为 2（如 0→2）。内层 tile 的 Layout 为 `shape: (2, 2), stride: (1, 2)`。

**外层排列（绿线）：** 红框在行方向排了 2 组（如 {0,1,2,3} 和 {4,5,6,7}，起始 offset 差为 4），在列方向排了 4 组（起始 offset 差为 8）。外层 stride：行方向为 4，列方向为 8。

合并后的层次化 Layout：**shape: ((2, 2), (2, 4)), stride: ((1, 4), (2, 8))**。

其中 shape 的四个数字 2, 2, 2, 4 分别表示内层 tile 行数、外层行方向 tile 数、内层 tile 列数、外层列方向 tile 数。
