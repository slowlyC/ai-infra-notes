...
现代 GPU 为长延迟操作(如全局内存访问)提供异步指令。异步操作允许内存传输与计算重叠执行, 
这种技术称为"流水线化"(pipelining)。

不同 GPU 厂商和架构的异步指令各有不同, 本教程聚焦于 NVIDIA GPU。 
在 NVIDIA GPU 上, 与直接读写寄存器的 `gl.load`/`gl.store` 不同, 异步拷贝是在全局内存和共享内存之间传输数据。
"""
...
# `cp.async`指令用于将数据从全局内存异步拷贝到共享内存, 我们用`cp.async`来重新实现 1D memcpy。
# Gluon 中通过 gl.allocate_shared_memory 分配共享内存, 分配时需要指定 layout。
# 共享内存的 layout 为 SwizzledSharedLayout, 主要用于减少 bank 冲突, 有时也是某些操作的硬性要求。

@gluon.jit
def memcpy_1d_cpasync_kernel(in_ptr, out_ptr, xnumel, XBLOCK: gl.constexpr):
    pid = gl.program_id(0)

    # 等价于 gl.SliceLayout(1, gl.BlockedLayout([1, 1], [32, 1], [4, 1], [1, 0]))
    # 但直接用 1D BlockedLayout 更简洁，当操作 2D tensor 时才需要 SliceLayout 来分离行列索引。
...
# 先写一个常规的同步版本 kernel 作为 baseline, 沿行方向划分 block。
@gluon.jit
...
# 即 32 个线程各处理 1 个 float, 正好对应 32 个 bank, 所以即使不 swizzle 也没有冲突。
...
#
# 思路: 
# - 异步读取下一个数据块的同时, 计算当前数据块。这样可以将内存传输(copy)与计算(add)进行重叠。

# 流水线深度(num_buffers):
# - 表示同时进行的异步拷贝数量. 
# - 当 copy 耗时是 add 的 N 倍时, 使用 num_buffers = N 个缓冲区, 就能达到最优的重叠效果。（前提是add可以异步执行）
...
#
#
# 上面场景假设copy和add都是可以异步执行的, 但在本节代码中add不能异步执行, 因此需要考虑以下情况: 
## ══════════════════════════════════════════════════════════════════════════════
...
#
#
# 【为什么Warmup/Cooldown阶段是 num_buffers-1 次迭代？】
# 提前"发射"num_buffers - 1个copy, 这样当 add(i) 完成时, copy(i+1) 刚好也完成, 下一轮的计算可以无缝衔接！
...
def elementwise_add_pipelined_kernel(  #
    ...
    for _ in range(gl.cdiv(ynumel, YBLOCK) - (num_buffers - 1)):
        ...
        # 等待最早的copy完成, 第一次循环的话就是等待copy(0)完成
...
# 但进一步增加流水线深度并不会产生更高的性能, 表明该kernel是memory-bound的。
#
# 【性能瓶颈分析】
...
# 1. 数据存储:
#    - a_val: 32 bits (1个float32寄存器)
#    - b_val: 32 bits (1个float32寄存器)
#    - c_val: 32 bits (1个float32寄存器)
#
# 2. 地址计算(GPU上的地址是64位的, 读写每个元素时都需要知道其在全局内存中的地址):
#    - a_ptrs: 64 bits (2个32位寄存器, 存储指针地址)
#    - b_ptrs: 64 bits (2个32位寄存器)
#    - c_ptrs: 64 bits (2个32位寄存器)
#
# 3. 掩码(Mask):
#    - mask: 用于边界检查, 每个元素需要 1 bit, 但通常占用完整寄存器
#
# 基于layout和分块大小, 分析其寄存器压力:
#
# Layout定义: BlockedLayout([1, 1], [1, 32], [1, 4], [1, 0])
# - Layout块大小 = [1x1x1, 1x32x4] = [1, 128], 即总线程数 = 128线程
# - size_per_thread = [1, 1]: 在layout块内每个线程拥有1x1个元素的寄存器
#
...
#
# 每个线程需要的寄存器(简化估算):
#   * 数据: 16个元素 x 3 (a,b,c) = 48个寄存器
#   * 地址: 16个元素 x 6 (a,b,c各2个) = 96个寄存器(因为这16个元素不连续, 需要存储16个地址)
#   * 掩码: 16个元素 ≈ 16个寄存器
...
# - 如果增大块大小(如64x128), 每个线程处理更多元素, 寄存器会溢出
