...
def memcpy_1d_tma_kernel(in_desc, out_desc, XBLOCK: gl.constexpr):
    ...
    # 注意: 异步 copy 和 TMA 存储的 Commit Group 是分开的, 互不影响.
    #
...
# 实际上, TMA版本的 Kernel 峰值寄存器使用率会保持在较低水平, 因为编译器会在内部循环中交错执行共享内存读取、加法计算和共享内存存储。
# 此时块大小的主要限制是共享内存容量。
...
# 详细计算过程如下:
# 情况 1: 128x128xf32 块 + 双重缓冲
# 单个块 = 128 x 128 x 4 bytes = 65,536 bytes = 64 KB
# 总共享内存需求 = A的缓冲 + B的缓冲 + 输出C的缓冲 = (64 KB x 2) + (64 KB x 2) + (64 KB x 1) = 320 KB

# 情况 2: 64x128xf32 块 + 三重缓冲
# 单个块 = 64 x 128 x 4 bytes = 32,768 bytes = 32 KB
# 总共享内存需求 = A的缓冲 + B的缓冲 + 输出C的缓冲 = (32 KB x 3) + (32 KB x 3) + (32 KB x 1) = 224 KB
