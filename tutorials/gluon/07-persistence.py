...
#   SM0: [0,0] [0,1] [0,2] ...
#   SM1: [1,0] [1,1] [1,2] ...   ← 同一行的 tile 分散在不同 SM, L2 缓存命中率低
#
# 分组调度 (GROUP_SIZE_M=8):
#   SM0: [0,0] [1,0] ... [7,0]
#   SM1: [0,1] [1,1] ... [7,1]   ← 同一列的 tile 在同一 SM, 共享 B 矩阵, L2 命中率高
...
#   B_buf[5]:  B0, B1, B2, B3, B4   ← 5 个, 多出 1 个给 epilogue 用
...
#   此时 producer % 5 指向的 B 缓冲区, 上一次 MMA 已经消费完它了(已经空闲)
#
#   假设 epilogue 时 producer = 10, producer % 5 = 0 → B0
#   代码中: c_buf = b_bufs.index(producer % (num_buffers + STEALB))
#   实际上代码会选择一个确保已经空闲的缓冲区
#
## kernel解析
# 假设的参数
# - num_buffers = 4
