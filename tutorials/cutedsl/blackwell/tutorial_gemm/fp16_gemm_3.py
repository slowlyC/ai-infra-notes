...
# 第四个 GEMM 教程, 在第三个教程 `fp16_gemm_2.py` (Warp Specialization) 基础上增加
# Static Persistent Tile Scheduler (静态持久化 tile 调度器).
...
"""
相较 `fp16_gemm_2.py`, 本 kernel 使用静态持久化 tile 调度器 (StaticPersistentTileScheduler).
Persistent kernel 是每个 CTA 持续驻留在 SM 上, 处理完一个 tile 后不退出, 而是从调度器获取下一个 tile 继续工作.
Static 指 tile 到 CTA 的分配方式是静态、确定的, 每个 CTA 根据自己的 block_idx 和 grid 算出负责哪些 tile。

注意: 若部分 SM 资源不可用, 静态调度器易受负载不均衡影响, 因此在下一示例 `fp16_gemm_3_1.py` 中引入动态调度器.

Persistent Kernel 的性能收益来自两个方面:

一是掩盖 prologue/epilogue 开销。 当一个 CTA 连续处理多个 tile 时, tile_i 的 epilogue 可以和 tile_{i+1} 的 
prologue 在不同 warp 中重叠执行。这正是 Warp Specialization + Persistent 组合的优势。
问题规模越大该部分收益越明显,因为 tile 越多,相邻 tile 之间的 prologue/epilogue 重叠机会就越多。

二是减少 CTA launch/teardown 开销。 少量 CTA 持续驻留,避免了大量 CTA 的调度与资源分配/释放。
...
ab_stages = 6
epi_stages = 2
acc_stages = 2  # [新增] 相对 gemm_2 的 1 改为 2, 为 accumulator pipeline 双缓冲 (配合 persistent 多 tile)

# [新增] 调度器: StaticPersistentTileScheduler
...
def kernel(
    ...
    # Multicast 参与者数 = 同行同列中发射 MMA 的线程数; 减 1 避免重复计数
    ...
    # [新增] barrier 初始化后 cluster 内 arrive (pipeline_init_arrive)
    pipeline_init_arrive(cluster_shape_mn=cluster_shape_mnk, is_relaxed=True)

    # 分配 SMEM (A/B/C tensor)
    ...
    # [新增] 开始工作前 cluster wait (pipeline_init_wait)
    pipeline_init_wait(cluster_shape_mn=cluster_shape_mnk)

    # [新增] 创建 tile 调度器并取得初始 work_tile
    ...
    if warp_idx == tma_warp_id:
        #
        # [新增] Persistent tile 调度外循环 (相对 gemm_2 的单 tile K 循环)
        #
        while work_tile.is_valid_tile:
            # 从 tile 调度器取得当前 tile 坐标
            ...
            # 按当前 MMA tile 的 M/N 索引 slice GMEM 视图
            # ((atom_v, rest_v), RestK)
            ...
            # 当前 output tile 上的 TMA load 内层循环 (K 维)
            ...
            # [新增] 前进到调度器上的下一个 work (下一个 output tile)
            tile_sched.advance_to_next_work()
            work_tile = tile_sched.get_current_work()

        # mbarrier_wait: 防止 cluster 内相互依赖的 thread block (TMA/MMA 同步语义下) 过早退出,
        # 避免过晚的 tcgen05 commit_arrive 非法
        ab_producer.tail()
    ...
    # MMA warp 分支
        ...
        # [新增] Persistent tile 调度外循环 ([新增] 使用 acc_stages 维的 tCtAcc_base 选当前 stage)
        while work_tile.is_valid_tile:
            if is_leader_cta:
                # 等待空的 accumulator 缓冲区
                acc_empty = acc_producer.acquire_and_advance()

                # 为当前 tile 选择 TMEM accumulator 视图
                # (MMA, MMA_M, MMA_N)
                ...
                for k_tile_idx in range(num_k_tiles):
                    ...
                    # 通知 A/B 缓冲区已消费, 可供下一轮 load
                    handle.release()

                # 通知 accumulator 已完成本 tile 计算
                acc_empty.commit()
    ...
    # Epilogue warps
        ...
        while work_tile.is_valid_tile:
            ...
            # 等待 accumulator 缓冲区已满 (本 tile 结果就绪)
            ...
            tCtAcc_epi = cute.flat_divide(
                tCtAcc[((None, None), 0, 0)],  # 取 MMA 子块 (0,0) 用于 epilogue 划分
                epi_tile,
            )
            ...
            for subtile_idx in cutlass.range(subtile_cnt):
                ...
                # 内存栅栏 + barrier, 确保 st.shared 对 TMA store 可见
...
# [新增] 由输出张量形状与 cluster 配置推导 PersistentTileScheduler 参数及 launch grid
def compute_grid(
    c: cute.Tensor,
    ...
    # 假设 mnk = 8192x8192x8192, mma_tiler_mnk = (256,256,64), use_2cta_instrs = True, cluster_shape_mnk = (2,1,1)
    # 从 (128, 256, 64) 取 M/N 维 → c_shape = (128, 256)
    c_shape = cute.slice_(mma_tiler_mnk, (None, None, 0))
    # gc 形状为 ((128, 256), (64, 32)), 前半 (128, 256) = 单个 tile 的元素, 后半 (64, 32) = M/N 方向各多少个 tile
    gc = cute.zipped_divide(c, tiler=c_shape)
    # 取 tile 内第 0 元素, 保留全部 tile 索引 → (64, 32), 即 M 方向 64 个 tile, N 方向 32 个 tile, 总共 2048 个 output tile
    num_ctas_mn = gc[(0, (None, None))].shape

    # problem_blocks_mnl = (64, 32, 1), cluster_shape_mnk = (2,1,1)
    tile_sched_params = utils.PersistentTileSchedulerParams(
        (*num_ctas_mn, 1), cluster_shape_mnk
    )
    # grid CTA 总数 = min(total_tiles, max_active_clusters) * cluster_size
    # 假设 max_active_clusters = 64, 则 grid = (min(2048, 64) * 2, 1, 1) = (128, 1, 1)
...
def run_dense_gemm(
    ...
    # 构造 K-major 张量 (torch 存储为 row-major)
