"""
分块缩放矩阵乘法 (Blocked-Scaled Matrix Multiplication)
======================================================
...
│  1位 │   E 位    │   M 位   │
└──────┴──────────┴──────────┘
Normal情况(E ≠ 0): 值= (-1)^sign x 2^(exp - bias) x (1 + mantissa/2^M)
                        ────────   ────────────────   ─────────────────
                        符号         指数部分            尾数部分
Subnormal情况(E=0): 值 = 2^(1-bias) x (0 + mantissa/2^M)
...
bias = 2^(2-1) - 1 = 1
value = (+1) x 2^1 x (1 + 1/2^1) = 1 x 2 x (1 + 0.5) = 3.0
...
换算成有效十进制位数 ≈ M x log10(2) ≈ M x 0.301
推导过程:
M 位二进制尾数 → 能区分 2^M 个不同的小数部分
n 位十进制数 → 能区分 10^n 个不同的数
要让两者等价: 2^M = 10^n
取对数: n = M x log10(2) = M x 0.301

fp32 (M=23): 23 x 0.301 ≈ 7 位有效十进制数
fp16 (M=10): 10 x 0.301 ≈ 3 位有效十进制数
bf16 (M=7):   7 x 0.301 ≈ 2 位有效十进制数
fp4  (M=1):   1 x 0.301 ≈ 0.3 位(基本没精度！)

各种格式对比
格式       总位数  Sign  Exponent  Mantissa   表示范围          有效数字位数
─────────────────────────────────────────────────────────────────────────────
fp32         32     1       8        23       ±3.4x10^38        ~7 位
fp16         16     1       5        10       ±65504            ~3 位
bf16         16     1       8         7       ±3.4x10^38        ~2 位
...
# base.shape          # 逻辑形状: (MN, K) - 表示有 MN x K 个 fp4 元素
...
# %%
# 我们可以通过软件流水线或 warp-specialization 来提升简单 block-scaled 矩阵乘法 kernel 的性能。
# 但在此之前, 我们可以优化处理 MMA scales 的方式。
#
# scales 沿内部维度(即 K 维度)是连续的。然而, 我们以 block 形状[BLOCK_M, BLOCK_K // VEC_SIZE] 加载 scales, 
# 即使 BLOCK_K 很大, 沿连续维度的加载大小也会小于 cache line 大小(128 字节)。
# 例如, 对于 BLOCK_K=256 和 MX scaling(VEC_SIZE=32), 沿连续维度的加载大小将只有 8 字节。
# 这会导致低效的全局内存访问合并、向量化和 L2 cache 利用率。
...
if __name__ == "__main__":
    ...
    for a_format, b_format in formats:
        A, A_scale, A_ref = random_quantized_tensor(M, K, a_format)
        B, B_scale, B_ref = random_quantized_tensor(N, K, b_format)
        # 当两个操作数都是 fp4 时使用 BLOCK_K=256, 否则使用 BLOCK_K=128。
...
if __name__ == "__main__":
    ...
    # 调用 warmup 来编译 kernel 并解析 constexprs。
    # 如果 kernel 在 cache 中, warmup 将不会运行, 可以设置 TRITON_ALWAYS_COMPILE=1 来强制重新编译。
